...
use App\Http\Requests\MMadoriRequest;
...
class MMadorisController extends Controller
{
     ...
     * 間取りマスタ登録処理
     *
     * @param MMadoriRequest $request リクエストパラメータ
     * @return \Illuminate\Http\JsonRespBonse
     */
    public function store(MMadoriRequest $request): \Illuminate\Http\JsonResponse
    {
        try {
            // パラメータから登録
            $result = MMadori::upsert($request);

            if ($result["code"] === 'fail') {
                $this->_status = 0;
                $this->_status_code = 500;
                $this->_messages[] = '間取名称の新規登録処理に失敗しました';
            }

            if ($result["code"] === 'err_name') {
                $this->_status = 0;
                $this->_status_code = 400;
                $this->_messages[] = '同名の間取名称データが存在しています。新規登録処理を実行できません';
            }
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function update(MMadoriRequest $request, int $id): \Illuminate\Http\JsonResponse
    {
        try {

            // パラメータから登録
            $result = MMadori::upsert($request, $id);
            ...
            if ($result["code"] === 'err_name') {
                $this->_status = 0;
                $this->_status_code = 400;
                $this->_messages[] = '同名の間取名称データが存在しています。更新処理を実行できません';
            }
