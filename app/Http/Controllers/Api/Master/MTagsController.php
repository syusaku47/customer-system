...
class MTagsController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            // パラメータから検索
            $results = MTag::search_list($request);
     ...
     * 関連タグマスタ登録処理
     *
     * @param MTagRequest $request リクエストパラメータ
     * @return \Illuminate\Http\JsonRespBonse
     */
    public function store(MTagRequest $request): \Illuminate\Http\JsonResponse
    {
        try {
            // パラメータから登録
            $result = MTag::upsert($request);

            if ($result["code"] === 'fail') {
                $this->_status = 0;
                $this->_status_code = 500;
                $this->_messages[] = '関連タグの新規登録処理に失敗しました';
            }
            ...
            if ($result["code"] === 'err_name') {
                $this->_status = 0;
                $this->_status_code = 400;
                $this->_messages[] = '同名の関連タグデータが存在しています。新規登録処理を実行できません';
            }
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function update(MTagRequest $request, int $id): \Illuminate\Http\JsonResponse
    {
        try {

            // パラメータから登録
            $result = MTag::upsert($request, $id);
            ...
            if ($result["code"] === 'err_name') {
                $this->_status = 0;
                $this->_status_code = 400;
                $this->_messages[] = '同名の関連タグデータが存在しています。更新処理を実行できません';
            }
