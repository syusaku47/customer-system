    ...
    private static function get_format_column($collection): Collection
    {
        ...
        foreach ($collection as $item) {
            $data = [
                ...
                'name' => $item->name, // 顧客見込みランク名称
                'abbreviation' => $item->abbreviation, // 顧客見込みランク略称
...
//          セッションからログインユーザーのcompany_idを取得
            ...
            //            ランク名、略式表示の重複チェック
            $tmp = MCustomerEstimatedRank::where('company_id', $company_id)
...
//            トランザクション
            ...
            if ($id) {
                // 更新
                $instance = MCustomerEstimatedRank::find($id);
                if (is_null($instance)) {
                    return ["code" => '404'];
//                    ログインユーザーのcompany_idと一致しているか
                ...
                // 登録
                $instance = new MCustomerEstimatedRank();
                $instance->company_id = $company_id;

//                内部ID(internal_id)の最大値取得
                $max_internal_id = MCustomerEstimatedRank::where('company_id', $company_id)->max('internal_id');
