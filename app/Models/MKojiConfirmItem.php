...
class MKojiConfirmItem extends ModelBase
{
    use HasFactory;

    // テーブル名はクラスの複数形のスネークケース（m_koji_confirm_items）
    // 主キーのデフォルト名はid
...
//        }
    ...
    private static function get_format_column($collection): Collection
    {
        ...
        foreach ($collection as $item) {
            $arr = $item->toArray();
            $data = [
                'id' => $arr['id'], // 工事確認項目マスタID
                'item' => $arr['item'], // 項目
                'caution' => $arr['caution'], // 注意書き
            ];
            $results->push($data);
        }
