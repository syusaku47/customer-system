 ...
 * @package App\Models
 */
class MLostorder extends ModelBase
{
    use HasFactory;
//    use HasCompositePrimaryKey;


    // テーブル名はクラスの複数形のスネークケース（m_lostorder）
    ...
    private static function get_format_column($collection): Collection
    {
        ...
        foreach ($collection as $item) {
            ...
            $data = [
                ...
                'lost_reason' => $item->lost_reason, // 失注理由
...
//          セッションからログインユーザーのcompany_idを取得
            ...
            //            重複チェック
            $tmp = MLostorder::where('company_id',$company_id)
                ->where("lost_reason", $arr['lost_reason'])->first();
...
//                    ログインユーザーのcompany_idと一致しているか
                ...
                // 登録
                $instance = new MLostorder();
                $instance->company_id = $company_id;

//                内部ID(internal_id)の最大値取得
                $max_internal_id = MLostorder::where('company_id', $company_id)->max('internal_id');
