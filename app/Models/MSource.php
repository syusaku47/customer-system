 ...
 * @package App\Models
 */
class MSource extends ModelBase
{
    use HasFactory;
//    use HasCompositePrimaryKey;


    // テーブル名はクラスの複数形のスネークケース（m_sources）
...
//          セッションからログインユーザーのcompany_idを取得
            ...
            //            重複チェック
            $tmp = MSource::where('company_id',$company_id)
...
//                    ログインユーザーのcompany_idと一致しているか
                ...
                // 登録
                $instance = new MSource();
                $instance->company_id = $company_id;

//                内部ID(internal_id)の最大値取得
                $max_internal_id = MSource::where('company_id', $company_id)->max('internal_id');
